...
const mapChars = (text, schema) => [...text].map((char) => schema[transliterateMap[char]] ?? char).join("");
...
const gammaNasalRules = (word, schema, checkCapitals = false) => {
    ...
    if (gamma.test(word))
        word = word.replace(gamma, schema.SMALL_GAMMA_NASAL + schema.SMALL_GAMMA);
    if (kappa.test(word))
        word = word.replace(kappa, schema.SMALL_GAMMA_NASAL + schema.SMALL_KAPPA);
    if (xi.test(word))
        word = word.replace(xi, schema.SMALL_GAMMA_NASAL + schema.SMALL_XI);
    if (chi.test(word))
        word = word.replace(chi, schema.SMALL_GAMMA_NASAL + schema.SMALL_CHI);
    if (checkCapitals) {
        ...
        if (cXi.test(word))
            word = word.replace(cXi, schema.CAPITAL_GAMMA_NASAL + schema.CAPITAL_XI);
        if (cChi.test(word))
            word = word.replace(cChi, schema.CAPITAL_GAMMA_NASAL + schema.CAPITAL_CHI);
    }
    return word;
};
const rhoRules = (word, schema, checkCapitals = false) => {
    ...
    if (/ρρ/.test(word))
        word = word.replace(/ρρ/, schema.SMALL_DOUBLE_RHO);
    if (checkCapitals) {
        if (/Ρ\u{0314}/u.test(word))
            word = word.replace(/Ρ\u{0314}/u, schema.CAPITAL_RHO + schema.ROUGH_BREATHING_MARK);
        if (/ΡΡ/.test(word))
            word = word.replace(/Ρ/, schema.CAPITAL_DOUBLE_RHO);
    }
    return word;
};
const diphtongRules = (word, schema, checkCapitals = false) => {
    ...
    if (eta.test(word))
        word = word.replace(eta, schema.SMALL_ETA + schema.SMALL_UPSILON_DIPTHONG);
    if (omicron.test(word))
        word = word.replace(omicron, schema.SMALL_OMICRON + schema.SMALL_UPSILON_DIPTHONG);
    if (iota.test(word))
        word = word.replace(iota, schema.SMALL_UPSILON_DIPTHONG + schema.SMALL_IOTA);
    if (checkCapitals) {
